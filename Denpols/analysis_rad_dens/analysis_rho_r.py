...
if len(sys.argv) != 6:
    print("usage is", sys.argv[0], "fold gen Lbackbone lp_density nskip")
    exit(1)
...
print("Path:",temppath)
print("Looking for",'/G{}{}*'.format(mygen,myLbb))
filelist.append(sorted(glob.glob(temppath+'/G{}{}*'.format(mygen,myLbb))))   
#print(filelist)
...
for item in flat_list:
    elems = item.strip(".lammpstrj").split("_"); 
...
jj = 0; _z = 0
totrep = 0
#print("filelist",flat_list[newlist.argsort()])
...
print("finished")
mypath="/home/manos/Desktop/analysis_radial_density/ANALYSIS/G{}".format(int(mygen))
norm = np.trapz(polymer.rhist[:,1], polymer.rhist[:,0]); polymer.rhist[:,1] = polymer.rhist[:,1]/norm; 
norm = np.trapz(polymer.thhist[:,1], polymer.thhist[:,0]); polymer.thhist[:,1] = polymer.thhist[:,1]/norm;

np.savetxt(mypath+"/cylindrical_radial_density_lp%d_G%d%d.dat" % (int(lp_density),int(mygen),int(myLbb)), polymer.rhist, fmt='%.8e') 
np.savetxt(mypath+"/cylindrical_angle_density_lp%d_G%d%d.dat" % (int(lp_density),int(mygen),int(myLbb)), polymer.thhist, fmt='%.8e')
