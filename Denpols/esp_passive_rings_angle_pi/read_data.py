...
##### Setting up Angles############
angle_harmonic = AngleCosine(bend=1.5, phi0=np.pi)
...
lj_seed=np.random.randint(10000)
# restore simulation temperature
system.thermostat.set_langevin(kT=1.0, gamma=1.0,seed=lj_seed)
system.integrator.run(10000)
#system.integrator.run(WARM_STEPS * 10)
outfile.close()
index_r=int(sys.argv[1])
#############################################################
#      Integration                                          #
#############################################################
outfile = open('polymer_equil_%d.vtf'%(index_r), 'w') 
vtf.writevsf(system, outfile)        
t_steps = int(1E9)
framestep=500
...
for t in range(t_steps):
    ...
    tostore[t,3]=box
    tostore[t,4]=dens
np.savetxt("log_%d.out"%(index_r),tostore,delimiter='\t',header=header)
#np.savetxt("log_noh.out",tostore,delimiter='\t')
checkpoint.save()
outfile.close()
