...
class UnitCell:
    ...
    @classmethod
    def hcp(cls):
        """Create a hexagonally-closed packed (hcp) unit cell.

        Returns:
            :class:`~.UnitCell`: A hexagonally-closed packed (hcp) unit cell
        """
        fractions = np.array([[0,0,0],[0.5,0.5,0],[0.5,5.0/6.0,0.5],[0.0,1.0/3.0,0.5]])
        return cls([1, 1, 1], fractions)
