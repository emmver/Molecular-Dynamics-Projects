...
for i in range(pos.shape[1]):
    ...
    sys.stdout.write("\r" + str(g))
    sys.stdout.write('\n')
np.savetxt(wdir+'g1.dat',(np.stack((time[1:],msds.mean(axis=0)[1:]),axis=-1)))


DPsm=96;DPlarge=250
data=np.loadtxt('data_equil/test_pos10000000.dat',skiprows=9)
...
data_small=data[np.where(data[:,1]==2)]#### Finds where particle type is 1 (small) ###########
data_large=data[np.where(data[:,1]==1)]#### Finds where particle type is 2 (large) ###########
...
for i in range(pos_coms.shape[1]):
    ...
    sys.stdout.write('\n')
    #sys.stdout.flush()
np.savetxt(wdir+'msd_coms.dat',(np.stack((time[1:],g3s.mean(axis=0)[1:]),axis=-1)))
