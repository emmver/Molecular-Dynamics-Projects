...
DPsm=96;DPlarge=250
data=np.loadtxt(wdir+'test_pos1100000.dat',skiprows=9)
...
msds = np.zeros((pos.shape[1], len(files)))
for i in range(pos.shape[1]):
    ...
    sys.stdout.write("\r" + str(g))
    sys.stdout.write('\n')
np.savetxt(wdir+'small_g1.dat',(np.stack((time[1:],msds.mean(axis=0)[1:]),axis=-1)))


#### COMs MSD #####

pos_coms=np.zeros((len(files),nsmall,3))
pos_coms=coms_times(pos,pos_coms,DPsm,nsmall,len(files))
#pos=np.zeros((len(files),npart,3))
g3s = np.zeros((nsmall, len(files)))
for i in range(pos_coms.shape[1]):
    ...
    sys.stdout.write('\n')
    #sys.stdout.flush()
np.savetxt(wdir+'small_msd_coms.dat',(np.stack((time[1:],g3s.mean(axis=0)[1:]),axis=-1)))
