...
for nca in [52]:#[10,30,50,70,90,100,110,130,150,170,190]:
	for dirnr in range(1):
		mpclist = [96,250]#[400,800]#
		...
		for u in alist:
			...
			for ii in range(Nlist[ch]):
				...
				pid+=1
				if Nlist[ch]==mpclist[0]:
					tid=1
				elif Nlist[ch]==mpclist[1]:
					tid=2
				data.append([pid,ch+1, tid, rvec[0],rvec[1],rvec[2]])
				data2.append([pid, tid, rvec[0],rvec[1],rvec[2]])
				...
				if ii==0:
					...
					pid_bonds+=1
					bonds.append([pid_bonds, 1, pid-1,pid])
				...
				if ii==0:
					...
					pid_angles+=1
					angles.append([pid_angles, 1, pid-2,pid-1,pid])
		...
		with open(outpath+'head','w') as of:
			...
			of.write(str(int(nca)*(96-1)+int(nc-nca)*(250-1))+' bonds\n')
			of.write(str(int(nca)*(96-2)+int(nc-nca)*(250-2))+' angles\n')
			of.write('\n')
			of.write('2 atom types\n')
